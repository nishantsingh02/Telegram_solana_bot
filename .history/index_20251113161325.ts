...
bot.action("export_private_key", async (ctx) => {
    try {
        ...
        const privateKeyBase58 = bs58.decode(keypair.secretKey);

        return ctx.reply(
            `🔐 **Your Private Key**  
        
⚠️ *Keep this secret! Anyone who has this can access your funds.*
...
\`${privateKeyBase58}\``,
    { parse_mode: "Markdown" }
        ...
        console.log(error);
        await ctx.reply("❌ **Failed to display your PrivateKey.**\nPlease try again.")
    }
})
...
bot.on(message("text"), (ctx) => {
    ...
    if(PENDING_REQUEST[userId]?.type === "SEND_SOL" && PENDING_REQUEST[userId].to && PENDING_REQUEST[userId].amount) {
        ...
        // if(!isValidPrivateKey(ctx.message.text)) {
        //     return ctx.reply("❌ Invalid private key format.")
        // }

        const keypair = getPrivateKey(PrivateKeyInput);

        if(!keypair) {
            return ctx.reply("❌ Invalid private key format. Please send a valid Base58 or JSON array private key.")
        }

        // transection logic using Immediately Invoked Async Function Expression.
        (async () => {
            try {
...
🔗 Explorer: https://explorer.solana.com/tx/${signature}?cluster=devnet`,
                ...
                console.log(error);
                ctx.reply("❌ Transaction failed. Please check your balance and try again.")
            }

            // clear pending request after txn
