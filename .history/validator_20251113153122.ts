...
export function isValidPrivateKey(privkey: string) {
    try {
        ...
        if(privkey.trim().startsWith("[")) {
            ...
            if(!arr.every((n: any) => Number.isInteger(n) && n>=0 && n<=255)) // ensures every element in the array is an integer and between 0 and 255 
                return false;
            secretKey = new Uint8Array(arr); // privkey is in this form "[12,34,...]"
        }
        // if the input privkey is in bs58 form
        else {
            const decoded = bs58.decode(privkey);
            if(decoded.length !== 64) return false; // must be 64 bytes
            secretKey = new Uint8Array(decoded) // priv key is from "dgjsi...."
        }
...
export function getPrivateKey(privkey: string): Keypair | false {
    try {
        let secretKey: Uint8Array;
