...
import { Connection, Keypair, LAMPORTS_PER_SOL, sendAndConfirmRawTransaction, SystemInstruction, Transaction } from "@solana/web3.js";
...
bot.on(message("text"), (ctx) => {
    ...
    if(PENDING_REQUEST[userId]?.type === "SEND_SOL" && PENDING_REQUEST[userId].to && PENDING_REQUEST[userId]) {

    }

    // transection logic
    async function sendSol() {
        const txn = new Transaction().add(SystemInstruction.transfer({
        fromPubkey,
        toPubKey,
        lamport
    }))
